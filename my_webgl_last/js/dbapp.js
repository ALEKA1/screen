import './tree/css/zTreeStyle.css'
import './tree/css/metroStyle/metroStyle.css'
import './tree/js/jquery.ztree.core-3.5.min.js'
import './tree/js/jquery.ztree.core-3.5.min.js'
import './tree/js/jquery.ztree.excheck-3.5.min.js'
var DBapp = {};
(function() {
	DBapp = {
		ui : {},
...
(function() {
    ...
    openTree : function(rows) {
    	...
    	for(var i = 0; i < rws.length; i++) {
    		var g = jQuery(rws[i]);
      	g.find(".cls-grid-col-tree").addClass("cls-menu-item-active");
      	g.find("." + this.RowChildCls).css("display", "block");
    	}
    },
    closeTree : function(rows) {
    	var rws;
    	if(rows) {
    		rws = rows;
    	} else {
    	...
    	for(var i = 0; i < rws.length; i++) {
    		var g = jQuery(rws[i]);
      	g.find(".cls-grid-col-tree").removeClass("cls-menu-item-active");
      	g.find("." + this.RowChildCls).css("display", "none");
    	}
    },
...
(function() {
  ...
  ComboTree.prototype = {
    type: "normal", //type分normal和extend
		isAjaxFilter: false,
    splitStr: "/n",
    name: "",
    valueField: "name",
    ...
    initList: function() {
      ...
      if(!this.list) {
        ...
        if(this.type == 'extend') {
          this.list.classList.add('extendList');
          var searchInput = '<div class="cls-menu-inputWrap">'+
                              '<input class="cls-form-field cls-menu-input" placeholder="Search" type="text">'
                            '</div>';
          $(this.list).append(searchInput);
          this.$extendInput = this.$list.find('.cls-menu-input');
        }
        if(this.listWidth) {
    ...
    createLvTree: function(store) {
      ...
      if(!self.isAjaxFilter) {
        if(self.type == 'extend') {
          DBapp.initFilterTree(self.tree, self.$extendInput);
        }
        else {
  ...
  $.fn.customTree = function(opt) {
    var tree = new ComboTree(this, opt, function() {
      var showValue = [];
      for( var i = 0; i < this.multValue.length; i++ ) {
        showValue.push(this.multValue[i]['name']);
      }
      this.$input.val(showValue.join(','));
      this.$el.val(showValue.join(','));
      this.grid.load(this.multValue);
    });
    tree.$input.attr('readonly','readonly');
    tree.doQuery = function(value) {
      var self = this;
      if(!self.isAjaxFilter && self.tree) {
      } else {
      	$.ajax({
          type : this.type || "get",
          dataType : this.dataType || "json",
          url : base.getAction(this.url) || "loginAction",
          data : this.param || {},
          cache : this.cache || false,
          success : function(data) {
    ...
    tree.clTreeData = function(nds) {
    	...
    	for(var i = 0; i < nds.length; i++) {
          ...
          if(nd.checked) {
            ...
            if( nd.check_Child_State > 0 && nd.isParent) {
              self.clTreeData(nd.children);
            }
            else if(check){
              self.removeMultValue(check);
