class Earth {
	constructor(opt) {
		...
		this.flightLineColor = {
			"0": new THREE.Color(0xdd380c),		
		  "1": new THREE.Color(0x154492),		
		  "2": new THREE.Color(0xAF0815),		
		  "3": new THREE.Color(0xdd380c),		
		  "4": new THREE.Color(0xAF0815),
		  //"5": new THREE.Color(0x32CD32)
		  "5": new THREE.Color(0x36D6BF)
		};
		this.HotspotColor = {
		  "0": new THREE.Color(0x154492),		
		  "1": new THREE.Color(0x154492),		
		  "2": new THREE.Color(0xAF0815),		
		  "3": new THREE.Color(0xdd380c),		
		  "4": new THREE.Color(0x0199CB),
		  "5": new THREE.Color(0x00FFA7)
		  //"5": new THREE.Color(0x32cd32)
		};
		this.radius = 100;
		this.segments = 60;
...
//  	}
  ...
  addSkyBox() {
  	var cube = new THREE.Mesh(
			new THREE.CubeGeometry(2000, 1000, 1),
			new THREE.MeshBasicMaterial({
  			color: 0x00FFA7,
  			//color: 0x6FD5F0,
  			map: THREE.ImageUtils.loadTexture("js/webGL/images/universe.jpg"),
  			//map: THREE.ImageUtils.loadTexture("/images/bg-login.png"),
  			side: THREE.BackSide,
  			opacity: 1.0,
  			transparent: true,
  			blending: THREE.AdditiveBlending
  ...
  addGlow() {
  	var moonGlow = new THREE.Mesh(
	        ...
	        glowColor: {
	          ...
	          value: new THREE.Color(0x00FFA7)
	        },
	        viewVector: {
	          type: 'v3',
  ...
  showEarth(img) {
    ...
    var uniforms = {
        ...
        value: new THREE.Color(0x0B553E)
  ...
  addLinght() {
    ...
    this.scene.add(hemiLight2);*/
  	...
  	var hemiLight = new THREE.PointLight(0x33CCFF, 2.2);
  	hemiLight.position.x = -50; 
  	hemiLight.position.y = 0;
  	hemiLight.position.z = 350;
  	//hemiLight.castShadow = true;
    this.scene.add(hemiLight);
    var hemiLight1 = new THREE.PointLight(0x33CCFF, 2.2);
  	hemiLight1.position.x = 50; 
  	hemiLight1.position.y = 0;
  	hemiLight1.position.z = 350;
  	//hemiLight.castShadow = true;
    this.scene.add(hemiLight1);
    var hemiLight2 = new THREE.PointLight(0x33CCFF, 2.5);
  	hemiLight2.position.x = 0; 
  ...
  render() {
    ...
    //this.rotateY += this.controllers.spin * 0.01;
  ...
  drawLine_type(start, end, lineType, type) {
        ...
        void main() {
          ...
          if(vPos >= time && vPos <= time + leng) {
            ...
            gl_FragColor = vec4(color * glowColor, 1.0);
	          ...
	          gl_FragColor = vec4(color * glowColor, 0.7);
				    gl_FragColor = gl_FragColor * texture2D(texture, gl_PointCoord);
	        }
          if(vPos == 5.0 || vPos == vMax - 5.0) {
            gl_FragColor = vec4(color * glowColor, 1.0);
    ...
    points.update = function() {
    	...
    	if(this.material.uniforms.time.value >= this.geometry.vertices.length) {
    		...
    		this.material.uniforms.time.value += 12.5;
    	}
  ...
  addFlightLine(start, end, lineType, type) {
  	...
  	if(sv["name"] == "浙江" || sv["name"] == "北京" || sv["name"] == "美国") {
  		this.addCity(sv);
		}
  	if(ev["name"] == "浙江" || ev["name"] == "北京" || ev["name"] == "美国") {
  		this.addCity(ev);
		}	
  ...
  loadFlightLine(data, lineType) {
  	...
  	for(var i = 0; i < data.length; i++) {
  		this.addFlightLine(data[i]["start"], data[i]["end"], data[i]["lineType"] !== undefined ? data[i]["lineType"] : lineType, data[i]["type"]);
  	}
  }
  removeAllFlightLine() {
