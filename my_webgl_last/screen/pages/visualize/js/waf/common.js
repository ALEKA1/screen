...
$.setDate = function(type){
  var sd, ed, result;
  if(type == 0){//近24小时
    sd = moment() - moment.duration('23:59:59');
    ed = moment();
  }else if(type == 1){//近7天
    sd = moment() - moment.duration('6.23:59:59');
    ed = moment();
  }else if(type == 2){//近30天
    sd = moment() - moment.duration('29.23:59:59');
    ed = moment();
  }else if(type == 3){//today
    sd = moment().startOf('day');
    ed = moment();
  }else if(type == 4){//thisWeek 周一0点开始
    sd = moment().startOf('week');
    ed = moment();
  }else if(type == 5) {//thisMonth
    sd = moment().startOf('month');
    ed = moment();
  }
  result = {
    startTime: moment(sd).format('YYYY-MM-DD HH:mm:ss'),
    endTime: moment(ed).format('YYYY-MM-DD HH:mm:ss')
  };
