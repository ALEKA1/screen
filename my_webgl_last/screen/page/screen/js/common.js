...
$.setDate = function(type){
  var sd, ed, result;
  if(type == 0){//近24小时
    sd = moment() - moment.duration('23:00:00');
    ed = moment();
    result = {
      startTime: moment(sd).format('YYYY-MM-DD HH:00:00'),
      endTime: moment(ed).format('YYYY-MM-DD HH:mm:ss')
    };
    return result;
  }else if(type == 1){//近7天
    sd = moment() - moment.duration('5.23:59:59');
    ed = moment();
  }else if(type == 2){//近30天
    sd = moment() - moment.duration('28.23:59:59');
    ed = moment();
  }else if(type == 3){//本日
  ...
  result = {
    startTime: moment(sd).format('YYYY-MM-DD 00:00:00'),
    endTime: moment(ed).format('YYYY-MM-DD HH:mm:ss')
  };
